...
gulp.task('image', function() {
    return gulp.src('mrb_dev/images/*')
        .pipe(imagemin())
...
gulp.task('js', function() {
    return gulp.src(['mrb_dev/js/**/*.*', 'mrb_dev/js/plugins/circliful.js', 'mrb_dev/js/*.js']) // al folder al feh al files bt3te
...
gulp.task('css', function() {
    return gulp.src('mrb_dev/scss/style.scss')
...
gulp.task("cssar", function() {
    return (
        gulp
        .src("mrb_dev/scss/style-rtl.scss")
        .pipe(sourcemaps.init()) //lazem yt7t b3d al source 3la tol
        .pipe(sass({ outputStyle: 'compressed' })) //hytl3 al output style md3'ot
        .pipe(prefix("last 2 versions")) //bygeb a5er 2 versions mn al brwosers
...
gulp.task('watch', function() {
    ...
    gulp.watch('mrb_dev/scss/**/*.scss', gulp.series('css')) //ay folder gwa al css w ay file amtddao css 
    gulp.watch("mrb_dev/scss/style-rtl.scss", gulp.series("cssar")); //ay folder gwa al css w ay file amtddao css

    gulp.watch(['mrb_dev/js/**/*.*', 'mrb_dev/js/plugins/circliful.js', 'mrb_dev/js/*.js'], gulp.series('js')) //ay folder gwa al css w ay file amtddao css 
    gulp.watch('mrb_dev/*.html', gulp.series('html'))
    gulp.watch('mrb_dev/images/*', gulp.series('image'))
